...
- 차트 색상/장 상태 UX 정리
  - 캔들/거래량 색상을 한국식으로 통일: `양봉=빨강`, `음봉=파랑`
  - 범례 샘플 색상도 실제 차트 색상과 동일하게 정렬
  - 장 종료 시 UI 상태를 `장 종료`로 표시하고, 5초 자동 폴링/실시간 WS 갱신을 중지
  - 장 종료 중에는 요약 상태만 저빈도(10분) 확인하고 필요 시 수동 새로고침 사용
...
장 상태 판정(MVP 규칙):
- 서버 기준(KST) 평일 `09:00 <= 현재시각 < 15:30`이면 `open`, 그 외 `closed`
- 공휴일/임시휴장 캘린더는 아직 반영하지 않음(주말 + 정규장 시간만 반영)
